...
fun StockChart(
    ...
    Canvas(modifier = modifier) {
        ...
        (0 until infos.size - 1 step 2).forEach { i ->
            ...
            // Using native Canvas as Compose Canvas cannot draw text/numbers
        ...
        val numbersToShow = 5f
        val spaceFromStart = 30f
        val priceStep = (upperValue - lowerValue / numbersToShow)
        (0..4).forEach { i ->
            drawContext.canvas.nativeCanvas.apply {
                drawText(
                    round(lowerValue + priceStep * i).toString(),
                    spaceFromStart,
                    size.height - spacing - i * size.height / numbersToShow,
                    textPaint
                )
            }
        }
